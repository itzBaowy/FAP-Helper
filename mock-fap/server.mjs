...
export async function startServer({port=8990,directory=process.env.FAP_MOCK_DATA_DIR || path.join(process.env.APPDATA || root,'FAPHelper-Mock'),publicOrigin='',accessKey=''}={}) {
  ...
  const server=http.createServer(async (req,res) => {
    ...
    try {
      ...
      const assets={'/':'index.html','/app.mjs':'app.mjs','/connection.mjs':'connection.mjs','/xlsx-importer.mjs':'xlsx-importer.mjs','/style.css':'style.css'};
      ...
      for await(const chunk of req) { size+=chunk.length; if(size>maxRequestBytes) return reply(413,{error:'Dữ liệu import vượt quá 64 MB.'}); chunks.push(chunk); }
