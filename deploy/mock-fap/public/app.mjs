...
function renderGroups(){
  ...
  for(const g of state?.groups??[]) $('group').add(new Option(`${g.subject} · ${g.classCode}`,g.id));
...
function renderSessions(){
  ...
  for(const s of sessions)$('session').add(new Option(`${s.date} · Slot ${s.slot}`,s.id));
...
function renderRows(){
  ...
  if(!session){if(state)message('Lớp chưa có buổi được lưu. Chốt buổi trong desktop rồi nhập lại FA26.json.');return;}
  const group=state.groups.find(g=>g.id===session.groupId),record=state.records[session.id];
  $('title').textContent=`${group.subject} · ${group.classCode}`;$('detail').textContent=`${session.date} · Slot ${session.slot}`;$('count').textContent=`${group.students.length} sinh viên`;
  for(const [i,s] of group.students.entries()){
    const tr=document.createElement('tr');tr.dataset.roll=s.roll;
    for(const [column,text] of [i+1,s.roll,s.name,s.email].entries()){
...
$('import').addEventListener('change',async()=>{
  ...
  try{if(file.size>10*1024*1024)throw Error('File vượt quá 10 MB.');state=await api('/api/import',{book:JSON.parse(await file.text()),revision:state?.revision??null});renderGroups();message(`Đã nhận ${state.groups.length} lớp và ${state.sessions.length} buổi. Điểm danh đã submit trên trang được giữ nguyên.`);}
...
try{state=await api('/api/state');renderGroups();message(state?'Dữ liệu đã tải. Chọn lớp và buổi cần điểm danh.':'Chọn FA26.json hoặc dữ liệu mẫu để bắt đầu.');}
